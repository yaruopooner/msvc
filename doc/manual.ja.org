...
# last updated : 2013/09/29.03:17:22
...
** 編集したヘッダファイルをセーブしようとすると "basic-save-buffer-2: Opening output file: invalid argument `HEADER-FILE-NAME`" となりセーブできない
   必ず発生するわけではなくヘッダファイルサイズが大きいソース？などで発生する。
...
** Emacsパッケージの設定
   ...
   msvc/.minimal-config-sample を参照してください。
...
** アクティブプロジェクトバッファ
   ...
   *MSVC Project<`db-name`>*
   ...
   バッファ名をマウスクリックするとバッファへジャンプ可能、カーソル移動未対応。そのうちやります。
